["Backpack On Chest", "1.0", "Allow users to ACE interact to put their backpack on their chest.", "DerZade"] call FNC_RegisterModule;

zade_boc_onChest = ["zade_boc_onChest", "Backpack to Chest", "modules\backpack_on_chest\data\actions\onchest_ca.paa", {[_player] call zade_boc_fnc_actionOnChest}, {!(missionNamespace getVariable ['zade_boc_disabled',false]) && !(backpack _player isEqualTo '') && ([_player] call zade_boc_fnc_chestpack isEqualTo '')}] call ace_interact_menu_fnc_createAction;
[typeOf player, 1, ["ACE_SelfActions", "ACE_Equipment"], zade_boc_onChest] call ace_interact_menu_fnc_addActionToClass;

zade_boc_onBack = ["zade_boc_onBack", "Backpack to Back", "modules\backpack_on_chest\data\actions\onback_ca.paa", {[_player] call zade_boc_fnc_actionOnBack}, {!(missionNamespace getVariable ['zade_boc_disabled',false]) && (backpack _player isEqualTo '') && !([_player] call zade_boc_fnc_chestpack isEqualTo '')}] call ace_interact_menu_fnc_createAction;
[typeOf player, 1, ["ACE_SelfActions", "ACE_Equipment"], zade_boc_onBack] call ace_interact_menu_fnc_addActionToClass;

zade_boc_swap = ["zade_boc_swap", "Swap Backpack", "modules\backpack_on_chest\data\actions\swap_ca.paa", {[_player] call zade_boc_fnc_actionSwap}, {!(missionNamespace getVariable ['zade_boc_disabled',false]) && !(backpack _player isEqualTo '') && !([_player] call zade_boc_fnc_chestpack isEqualTo '')}] call ace_interact_menu_fnc_createAction;
